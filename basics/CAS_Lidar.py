...
BASELINE_ALPHA = 0.1 
# smoothing for baseline distance
# Controls how fast that baseline changes when the measured distance changes.

DOOR_DELTA = 0.40 # m: side distance must jump this much above reference
DOOR_MIN_RANGE = 1.20 # m: side distance must be at least this to be considered door

# Turning to/from doorway

TURN_RATE = 3 # rad/s 
DOOR_TURN_DURATION = 1.5 # time to swing toward door, 90 degree turn
RETURN_TURN_DURATION = 1.5 # time to swing back 
...
def detect_door(left_d, right_d, base_left, base_right): 
    ...
    if (left_d is not None) and (base_left is not None):
        if (left_d - base_left > DOOR_DELTA) and (left_d > DOOR_MIN_RANGE): 
            return 'L'
    
    # If the doorway is on the right, return R 
    if (right_d is not None) and (base_right is not None):
        if (right_d - base_right > DOOR_DELTA) and (right_d > DOOR_MIN_RANGE): 
            return 'R'
        ...
        while True: 
            ...
            # Define NAVIGATE 
                ...
                if state == END_OF_HALL: 
                    ...
                    if door is not None: 
                        door_side = door
                        state = TURN_TO_DOOR
                        state_start_time = time.time() # start timer
                        print(f"Door detected on {door_side}, entering TURN_TO_DOOR")
                        x_dot, theta_dot = 0.0, 0.0 
            ...
            # Define ENGAGE_ROOM
                ...
                if not engaged_once: # shooter code hasn't run yet
                    ...
                    engage_once = True # shooter code has been run
